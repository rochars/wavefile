...
describe('create 48-bit PCM wave files from scratch', function() {
    ...
    wav.fromScratch(1, 44100, '48', [0, -140737488355328, 140737488355327, 4]);

    let fs = require('fs');
    fs.writeFileSync("./test/files/out/48-bitPCM-441kHz-mono-fromScratch.wav", wav.toBuffer());
    ...
    it('samples should be the same as the args', function() {
        assert.deepEqual(wav.data.samples, [0, -140737488355328, 140737488355327, 4]);
    });
