...
describe('Upsample a 16bit 8kHz file', function() {
  ...
  wav.toSampleRate(16000, {method: 'lanczos'});

  // Write the file
  fs.writeFileSync(
    path + "/out/to-sample-rate/lanczos-16-bit-16001kHz-noBext-mono.wav",
    wav.toBuffer());
...
describe('Downsample a 32-bit 44.1kHz log sine sweep', function() {
  ...
  // Write the file
  fs.writeFileSync(
    path + "/out/to-sample-rate/lanczos-32fp-16000Hz-mono-chirp-1-22050-log.wav",
    wav.toBuffer());
});
...
describe('Upsample a 32-bit 44.1kHz log sine sweep', function() {
  ...
  // Convert to another sample rate
  wav.toSampleRate(96000, {method: 'lanczos'});

  // Write the file
  fs.writeFileSync(
    path + "/out/to-sample-rate/lanczos-32fp-96000Hz-mono-chirp-1-22050-log.wav",
    wav.toBuffer());
});
...
describe('Downsample a 32-bit 44.1kHz linear sine sweep', function() {
  ...
  // Write the file
  fs.writeFileSync(
    path + "/out/to-sample-rate/lanczos-32fp-16000Hz-mono-chirp-1-22050-linear.wav",
    wav.toBuffer());
});
...
describe('Upsample a 32-bit 44.1kHz linear sine sweep', function() {
  ...
  // Write the file
  fs.writeFileSync(
    path + "/out/to-sample-rate/lanczos-32fp-96000Hz-mono-chirp-1-22050-linear.wav",
    wav.toBuffer());
});
...
describe('Downsample a 16-bit 44.1kHz log sine sweep', function() {
  ...
  // Write the file
  fs.writeFileSync(
    path + "/out/to-sample-rate/lanczos-16bit-16000Hz-mono-chirp-1-22050-log.wav",
    wav.toBuffer());
});
...
describe('Upsample a 16-bit 44.1kHz log sine sweep', function() {
  ...
  // Write the file
  fs.writeFileSync(
    path + "/out/to-sample-rate/lanczos-16bit-96000Hz-mono-chirp-1-22050-log.wav",
    wav.toBuffer());
});
...
describe('Downsample a 16-bit 44.1kHz linear sine sweep', function() {
  ...
  // Write the file
  fs.writeFileSync(
    path + "/out/to-sample-rate/lanczos-16bit-16000Hz-mono-chirp-1-22050-linear.wav",
    wav.toBuffer());
});
...
describe('Upsample a 16-bit 44.1kHz linear sine sweep', function() {
  ...
  // Write the file
  fs.writeFileSync(
    path + "/out/to-sample-rate/lanczos-16bit-96000Hz-mono-chirp-1-22050-linear.wav",
    wav.toBuffer());
});
...
describe('Downsample a 16bit 44.1kHz file', function() {
  ...
  // Write the file
  fs.writeFileSync(
    path + "/out/to-sample-rate/lanczos-song1-16kHz.wav",
    wav.toBuffer());
});
...
describe('Upsample a 16bit 44.1kHz file', function() {
  ...
  // Write the file
  fs.writeFileSync(
    path + "/out/to-sample-rate/lanczos-song1-96kHz.wav",
    wav.toBuffer());
});
